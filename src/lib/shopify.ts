...
export const SHOPIFY_STORE_PERMANENT_DOMAIN =
  "my-creative-hub-ybq3q-f4vfnqfe.myshopify.com";
export const SHOPIFY_STOREFRONT_URL = `https://${SHOPIFY_STORE_PERMANENT_DOMAIN}/api/${SHOPIFY_API_VERSION}/graphql.json`;
export const SHOPIFY_STOREFRONT_TOKEN = "dee3ebfdd43bd5e51612967adcfce5e2";
...
export async function storefrontApiRequest(query: string, variables: Record<string, unknown> = {}) {
  ...
  if (response.status === 402) {
    toast.error("Shopify : paiement requis", {
      description:
        "L'accès à l'API Shopify nécessite un forfait actif. La boutique doit passer à un forfait payant.",
    });
    return;
  }
